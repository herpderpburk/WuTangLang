...
public class Message extends WutObject {
    ...
    private final WutObject cachedResponse;
    //Operator operator;

    public Message(String name, String file, int line, WutObject cachedResponse) {
        super(WutObject.message);
        this.name = name;
        this.file = file;
        this.line = line;
        this.next = null;
    ...
    public WutObject evalOn(WutObject on, WutObject base) throws WutException {
        ...
        if (this.cachedResponse == null) {
            try {
                ...
                if (response == null) response = WutObject.nil;
            } catch (Exception e) {
                // TODO Handle catching exception
                if (e instanceof WutException && e.getCause() != null) throw (WutException) e;
                throw new WutException(String.format("%s in %s:%d", e.getMessage(), file, line), e);
            }
        } else {
    ...
    public WutObject evalOn(WutObject on) throws WutException {
        return evalOn(on, on);
    }
    ...
    static public Message parse(String s, String code) throws ParsingException {
